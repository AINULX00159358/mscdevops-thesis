...
\textbf{Automation:}\\ Using GitHub workflow, the process is automated to build updated code and container image to docker repository. Refer ".workflow" folder in "Knative Code base" 
\href{https://github.com/AINULX00159358/InvoiceMgrKnative/blob/main/.github/workflows/buildAndPush.yml}  
{.github/workflows/buildAndPush.yaml}
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item Avoid POD's image pull policy as "Always"
...
\begin{figure}[h]
    \centering
    \includegraphics[width=1.0\linewidth]{images/AzurePerfCapture/eventgrid-topic-latency.PNG}
...
\begin{figure}[h]
    \centering
    \includegraphics[width=1.0\linewidth]{images/AzurePerfCapture/invoicegenerator-event-latency.PNG}
...
\subsection{Metrics: First Initial Deployment}
...
\subsection{Metrics: Warm Disk Start}
...
\subsubsection{Azure's Hosting plan}
Azure's Basic Serverless Hosting Plan is \textbf{"Consumption Plan"}. The Consumption plan scales application on traffic load factor. Customers are charged based on the compute resources utilized. The Compute instances for Functions hosts are dynamically added and removed based on the traffic load factor.

Azure also provides different Hosting Options:
\begin{itemize}
    \item ASE : Azure Service Environment, which has high degree of isolation and scalability.
...
\begin{quote}
    \textit{Knative is best for applications that generate a variable number of trigger events that, over time, stay within established limits. While a single application can meet this requirement, it's more likely met by a combination of three or more applications whose triggers aren't synchronized.}
    \\
    ( \cite{Nolle_2020} )
\end{quote}
\par
...
\section{Conclusion}
Knative is a Cloud-Native alternative to traditional Serverless offering by public cloud providers.
\hfill \break
However, since it offered over Kubernetes and Service Mesh, cost of maintaining the cluster is expensive, both in terms of IT resources, maintenance and serviceability cost, requiring a team of dedicated SRE.
\par
Thus, Knative is a better alternative, only for companies who already have Kubernetes and Service Mesh in their technology stack.
\par
Azure Functions will not offer a Cloud-Native solution. Companies can continue using Azure Function to avoid the overheads of maintaining the Kubernetes infrastructure, but will be vendor-locked towards Azure products.
